...
type GetDataSetStateType = React.Dispatch<React.SetStateAction<UserType[] | undefined>> | React.Dispatch<React.SetStateAction<OfferType[] | undefined>>

export const getData = (recource: string, setData: GetDataSetStateType): Promise<any> => {
    return axios("http://localhost:3000/" + recource)
    .then((response) => {
...
export const postOrder = (user: UserState, totalPrice: number, validCode: boolean) => {
    const navigate = useNavigate();
  ...
  export const getCodes = (code: string, setValidCode: (value: React.SetStateAction<boolean>) => void) => {
